...
function saveMessage(messageText) {
  ...
  .catch(function(error) {
    console.error('Error writing new messages to db', error);
  });
}
...
function loadMessages() {
  ...
  // create query to load the latest 12 messages and listen for new ones
...
function saveImageMessage(file) {
  ...
  // 1: add a message with loading icon
  ...
  .then(function(messageRef) {
    ...
    return firebase.storage().ref(filePath).put(file).then(function(fileSnapshot) {
      ...
      return fileSnapshot.ref.getDownloadURL().then(function(url) {
        // 4: update the message with image url
